...
You need to import `get_cfg_defaults()` from `config.py` for parameter setting, and use `cfg.merge_from_file()` to modify parameter for different version. Cooperate with python logging, it can save all setting and log of each version automatically.
